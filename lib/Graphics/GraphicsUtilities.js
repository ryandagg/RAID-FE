var RenderConstants = require('../RenderConstants');
...
module.exports = {
	...
	animateGlyphMove: function(renderGL, startPoint, endPoint, currAnimationFrame, glyph, fontStyle, color){
		var glyphAnimatedPos = this.linearlyInterpolate(startPoint, endPoint, currAnimationFrame, RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT);
	...
	animateGlyphMeleeAttack: function(renderGL, attackerPos, targetPos, currAnimationFrame, glyph, fontStyle, color){
		...
		var oneThirdFrameCnt = Math.floor(RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT / 3),
			twoThirdsFrameCnt = Math.floor(2 * RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT / 3);
	...
	animateGlyphRangedAttack: function(renderGL, attackerPos, targetPos, currAnimationFrame, radius, color){
		var projectileAnimatedPos = this.linearlyInterpolate(attackerPos, targetPos, currAnimationFrame, RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT);
		renderGL.drawCircle(
			{
	...
	animateGlyphMagicAttack: function(renderGL, attackerPos, targetPos, currAnimationFrame, radius, color){
		var projectileAnimatedPos = this.linearlyInterpolate(attackerPos, targetPos, currAnimationFrame, RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT),
			animPercentComplete = currAnimationFrame / RenderConstants.GAMERUNNER_ANIMATION_FRAME_CNT;
		renderGL.drawCircle(
			{
				center: projectileAnimatedPos,
